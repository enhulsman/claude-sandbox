...
# ── Bwrap Stub Cleanup ──────────────────────────────────────
# Claude Code's internal bwrap creates 0-byte read-only (mode 444) files as
# mount-point stubs in writable directories. These persist after sessions and
# pollute git status. We hide them during sessions and clean them after.
_CS_BWRAP_STUB_PATTERNS=(
  .bash_profile .bashrc .profile .zprofile .zshrc .zshenv
  .gitconfig .gitmodules .mcp.json .ripgreprc
  HEAD config objects refs
  .idea .vscode  # bwrap creates files (not dirs) as mount stubs
)
_CS_GIT_EXCLUDE_FILE=""
_CS_SENTINEL_BEGIN="# >>> claude-sandbox stub exclusions >>>"
_CS_SENTINEL_END="# <<< claude-sandbox stub exclusions <<<"
# Known limitations:
# - Concurrent sessions: identical sentinel blocks mean one session's cleanup
#   strips another's block. Stubs briefly visible in git status. No data loss.
# - Crash mid-write: orphan sentinel begin line is deleted, but partial patterns
#   between it and EOF persist as harmless dead lines in info/exclude.
...
# ── Bwrap Stub Helpers ────────────────────────────────────────
# strip_sentinel_block: Remove our injected block from a file.
# Handles matched pairs (normal) and orphan openers (crash during write).
strip_sentinel_block() {
  local file="$1"
  [[ -f "$file" ]] || return 0

  local has_begin has_end
  has_begin=$(grep -c "^${_CS_SENTINEL_BEGIN}$" "$file" 2>/dev/null) || true
  has_end=$(grep -c "^${_CS_SENTINEL_END}$" "$file" 2>/dev/null) || true

  if [[ "$has_begin" -gt 0 && "$has_end" -gt 0 ]]; then
    # Normal: matched pair — safe to range-delete
    sed -i "/^${_CS_SENTINEL_BEGIN}$/,/^${_CS_SENTINEL_END}$/d" "$file" 2>/dev/null || true
  elif [[ "$has_begin" -gt 0 ]]; then
    # Unclosed sentinel (crash during write) — delete only the orphan line
    # to avoid eating everything from the sentinel to EOF
    sed -i "/^${_CS_SENTINEL_BEGIN}$/d" "$file" 2>/dev/null || true
  fi
  # If neither sentinel present, nothing to do
}

# setup_git_exclude: Inject stub patterns into .git/info/exclude so bwrap
# mount-point stubs are invisible to git status during the session.
setup_git_exclude() {
  local launch_dir="$1"

  # Linux-only (stubs are a bwrap artifact)
  [[ "${CLAUDE_SANDBOX_IS_LINUX:-0}" == "1" ]] || return 0
  # Claude mode only (--exec doesn't create stubs)
  [[ ${#EXEC_CMD[@]} -eq 0 ]] || return 0

  # Must be in a git work tree
  git -C "$launch_dir" rev-parse --is-inside-work-tree &>/dev/null || return 0

  # Find the shared .git directory (handles worktrees correctly)
  local git_common_dir
  git_common_dir=$(git -C "$launch_dir" rev-parse --git-common-dir 2>/dev/null) || return 0
  # Resolve relative paths (worktrees return relative paths)
  if [[ "$git_common_dir" != /* ]]; then
    git_common_dir="$launch_dir/$git_common_dir"
  fi

  # Compute relative prefix from repo root for correct gitignore anchoring
  local toplevel
  toplevel=$(git -C "$launch_dir" rev-parse --show-toplevel 2>/dev/null) || return 0
  local rel_prefix=""
  if [[ "$launch_dir" != "$toplevel" ]]; then
    rel_prefix="${launch_dir#"$toplevel"/}/"
  fi

  # Ensure info/exclude exists
  mkdir -p "$git_common_dir/info"
  touch "$git_common_dir/info/exclude"

  local exclude_file="$git_common_dir/info/exclude"

  # Strip any stale block from a previous crashed session
  strip_sentinel_block "$exclude_file"

  # Build and append the sentinel block
  local block="${_CS_SENTINEL_BEGIN}"$'\n'
  block+="# Auto-generated by claude-sandbox. Removed on session exit."$'\n'
  for pattern in "${_CS_BWRAP_STUB_PATTERNS[@]}"; do
    block+="/${rel_prefix}${pattern}"$'\n'
  done
  block+="${_CS_SENTINEL_END}"

  # Ensure file ends with newline before appending sentinel block
  if [[ -s "$exclude_file" ]] && [[ "$(tail -c 1 "$exclude_file")" != "" ]]; then
    printf '\n' >> "$exclude_file"
  fi

  # Single append to minimize partial-write window
  printf '%s\n' "$block" >> "$exclude_file"

  # Store path for cleanup
  _CS_GIT_EXCLUDE_FILE="$exclude_file"
}

# cleanup_bwrap_stubs: Remove 0-byte, mode-444 stub files left by bwrap.
cleanup_bwrap_stubs() {
  # Linux-only
  [[ "${CLAUDE_SANDBOX_IS_LINUX:-0}" == "1" ]] || return 0
  # Claude mode only
  ...
  local launch_dir="${LAUNCH_DIR:-}"
  [[ -n "$launch_dir" ]] || return 0

  local count=0
  for pattern in "${_CS_BWRAP_STUB_PATTERNS[@]}"; do
    local file="$launch_dir/$pattern"
    [[ -e "$file" ]] || continue

    # Only delete if BOTH: size == 0 AND perms == 444
    local size perms
    size=$(stat -c %s "$file" 2>/dev/null) || continue
    perms=$(stat -c %a "$file" 2>/dev/null) || continue

    if [[ "$size" == "0" && "$perms" == "444" ]]; then
      rm -f "$file" 2>/dev/null && {
        echo "  Cleaned bwrap stub: $pattern" >> "${SESSION_DIR:-/dev/null}/session.log" 2>/dev/null || true
        ((count++)) || true
      }
    fi
  done
  ...
  if [[ $count -gt 0 ]]; then
    echo "  Cleaned $count bwrap mount-point stub(s)"
  fi
}
...
# Used for git check, git exclude injection, and stub cleanup
...
cleanup() {
  ...
  # Strip sentinel block from .git/info/exclude
  if [[ -n "${_CS_GIT_EXCLUDE_FILE:-}" ]]; then
    strip_sentinel_block "$_CS_GIT_EXCLUDE_FILE"
  fi
  # Remove bwrap mount-point stubs from disk
  cleanup_bwrap_stubs
...
# Hide bwrap stubs from git status during session
setup_git_exclude "$LAUNCH_DIR"
